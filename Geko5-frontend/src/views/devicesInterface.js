...
import { getColumnsDeviceInterface } from "../tools/column";


export default function DevicesInterface(props) {
    ...
    const loadData = async () => {
        ...
        try {
            const data = await ServerProxy.getDevicesInterface({
                ...localFilter
            });
    ...
    const onDelete = async function (id) {
        ...
        try {
            await ServerProxy.deleteDeviceInterface(id);
            await loadData();

            props.setAlert(prevState => ({
                ...prevState,
                message: strings.status200,
                hide: 1,
                severity: "success"
            }));
        } catch (error) {
    ...
    const onDeleteAll = async () => {
        ...
        try {
            await ServerProxy.deleteMultipleDevicesInterface(elementSelected);
            await loadData();
            props.setAlert(prevState => ({
                ...prevState,
                message: strings.status200,
    ...
    const onDetailOkAction = async function (obj) {
        try {
            if (obj?.id) {
                await ServerProxy.editDeviceInterface(obj);
            } else if (obj) {
                await ServerProxy.addDeviceInterface(obj);
            }
    ...
    const columns = getColumnsDeviceInterface(isFullScreen ,  user,  onDetailOkAction, setDetail, setActionFuncion, onDelete);
    ...
    useEffect(() => {
        document.title = `${strings.deviceInterfaces} - Wisnam`;
    ...
    return (
        ...
        <>
            ...
            {detail.open && (
                <DeviceInterfaceDetail load={load} setLoad={setLoad} setAlert={props.setAlert} detail={detail} onOkAction={onDetailOkAction} onCloseAction={() => setDetail({ id: 0, open: false })} />
            )}
                ...
                <Box sx={{ flexShrink: 0, mb: 2, mt: { sm: 5, lg: 2 } }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                            ...
                            <HeaderPage title={strings.deviceInterfaces} titleTwo={strings.Asset_management} />
                                ...
                                <Box sx={{ display: "flex" }}>
                                    {(user !== null && user?.role !== "operator") && (
                                        <Button onClick={() => setDetail({ id: 0, open: true })} sx={{ height: "32px", backgroundColor: "primary", textTransform: "none", fontSize: "14px", fontWeight: 400, mr: 1, '&:hover': { backgroundColor: "primary", color: "white" } }} variant="contained" >
                                            + {strings.addNewDeviceInterface}
                                        </Button>
                                    )}

                                    <ExportMenu alert={alert} setAlert={props.setAlert} table={"device_interface"} />
